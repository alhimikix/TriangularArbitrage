  ...
  */
  ...
  for (let i=0;i<bpairs.length;i++){
    ...
    if (akeys[bPairTicker.key]){
      ...
      let stepC = CurrencyCore.getCurrencyFromStream(stream, match.key, keys.a);

      // only do this if we definitely found a path. Some paths are impossible, so will result in an empty stepC quote.
      if (stepC){
        keys.c = match.key;

        let comparison = CurrencyCore.getArbitageRate(stream, keys.a, keys.b, keys.c);
...
CurrencyCore.getDynamicCandidatesFromStream = (stream, options)=>{
  ...
  for (let i=0;i<options.paths.length;i++){
    let pMatches = CurrencyCore.getCandidatesFromStreamViaPath(stream, options.start, options.paths[i]);
...
CurrencyCore.getBTCETHCandidatesFromStream = (stream)=>{
  ...
  let apairs = stream.markets.BTC;
  let bpairs = stream.markets.ETH;
  ...
  */
  ...
  for (let i=0;i<bpairs.length;i++){
    ...
    if (akeys[bPairTicker.key]){
      ...
      let rate = CurrencyCore.getArbitageRate(stream, keys.a, keys.b, keys.c);
      let triangle = {
        a: keys.a,
        b: keys.b,
        c: keys.c,
...
CurrencyCore.simpleArbitrageMath = (stream, candidates)=>{
  ...
  let a = candidates['BTCUSDT'];
  let b = candidates['ETHUSDT'];
  let c = candidates['ETHBTC'];
