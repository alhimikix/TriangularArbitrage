...
PairRanker.prototype.getPairRanking = (candidates, pairRanks, ctrl) => {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    let id = candidate['a_step_from'] + candidate['a_step_to'] + candidate['b_step_to'] + candidate['c_step_to'];
    let date = new Date;
    let pair = {
      id: id,
...
PairRanker.cleanPairingArray = (pairRanks) => {
  // Cleaned array
  return pairRanks.filter(function (pair) {
...
PairRanker.getTopPairs = (pairToCheck, pairRanks) => {
  ...
  let check = false;
  let id = pairToCheck['a_step_from'] + pairToCheck['a_step_to'] + pairToCheck['b_step_to'] + pairToCheck['c_step_to'];
  let pairsToCheck = pairRanks.filter(function(pair) {
    return pair.id === id;
  });
  let rate = 0;
